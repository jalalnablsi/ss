  ...
  return (
    <div className="flex flex-col items-center justify-between h-full w-full pt-4 pb-28 px-4 relative overflow-hidden">
      ...
      >
        {/* Main Coin */}
        <motion.div
          whileTap={{ scale: 0.92, rotateX: 15, rotateY: -15 }}
          transition={{ type: "spring", stiffness: 400, damping: 15 }}
          className="relative z-10 w-72 h-72 rounded-full cursor-pointer select-none perspective-1000"
        >
          {/* Outer Glow */}
          <div className="absolute inset-0 rounded-full bg-yellow-500/20 blur-2xl" />
          
          {/* Coin Body */}
          <div className="w-full h-full rounded-full bg-gradient-to-br from-yellow-300 via-yellow-500 to-amber-700 p-1 shadow-[0_10px_40px_rgba(217,119,6,0.5),inset_0_4px_10px_rgba(255,255,255,0.5)] relative overflow-hidden flex items-center justify-center">
            {/* Inner Ring */}
            <div className="w-[90%] h-[90%] rounded-full border-4 border-yellow-200/30 bg-gradient-to-br from-yellow-400 to-amber-600 flex items-center justify-center shadow-[inset_0_0_20px_rgba(0,0,0,0.2)]">
              {/* Coin Symbol */}
              <span className="text-8xl font-black text-yellow-100 drop-shadow-[0_4px_4px_rgba(0,0,0,0.3)]">
                T
              </span>
            </div>
            
            {/* Shine effect */}
            <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/30 to-transparent -rotate-45 translate-x-[-100%] animate-[shimmer_3s_infinite]" />
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {floatingNumbers.map(num => (
            <motion.div
              ...
              animate={{ opacity: 0, y: num.y - 160, scale: 1.5 }}
              exit={{ opacity: 0 }}
              transition={{ duration: 0.8, ease: "easeOut" }}
              className="absolute z-50 text-4xl font-black text-white pointer-events-none drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)]"
              style={{ left: 0, top: 0 }}
            >
              +{num.value}
            </motion.div>
      ...
      <div className="w-full max-w-sm px-4 z-10 space-y-3">
        ...
        <div className="h-6 w-full bg-[#111] rounded-full overflow-hidden border border-white/10 p-1 shadow-inner relative">
          ...
          >
            {/* Animated shine on the energy bar */}
