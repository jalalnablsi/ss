...
SELECT COUNT(*) as count FROM ad_watches WHERE telegram_id = ? AND watched_at >= ?,
...
UPDATE users SET coins = coins + 1500, challenge_coins = COALESCE(challenge_coins, 0) + 1500, referrals_activated = referrals_activated + 1, referral_coins_earned = COALESCE(referral_coins_earned, 0) + 1500 WHERE telegram_id = ?,
[user.referred_by]
);

// ✅ التحقق الآمن متعدد المستويات (تم إصلاح الخطأ)
...
await executeD1(
UPDATE users SET coins = ?, challenge_coins = ?, energy = ?, total_taps = ?, tap_multiplier = ?, tap_multiplier_end_time = ?, auto_bot_active_until = ?, ads_watched_today = ?, last_ad_watch_date = ?, last_update_time = ? WHERE telegram_id = ?,
[
finalCoins,
finalChallengeCoins,
finalEnergy,
